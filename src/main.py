...
def _run_python(module: str, cfg_path: Path):
...
def main():
    ...
    grp.add_argument("--full-experiment", action="store_true", help="Run full publication-grade experiment")
    args = parser.parse_args()

    cfg_file = CFG_DIR / ("smoke_test.yaml" if args.smoke_test else "full_experiment.yaml")

    # 1) preprocessing – idempotent
    _run_python("src.preprocess", cfg_file)

    # 2) training / evaluation
    _run_python("src.train", cfg_file)
